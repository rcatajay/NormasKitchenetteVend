...
def render_receipt(items, total):
    ...
    html = (
        "<style>"
        ".receipt{font-family:monospace;font-size:16px;line-height:1.35;}"
        ...
        ".receipt-right{min-width:80px;text-align:right;white-space:nowrap;}"
...
def load_store_from_database():
    ...
    for _, name, category, price, stock in menu_rows:
        if stock > 0:
            store.add_food(FoodItem(name, category, price))

    return store, menu_rows
...
store, menu_rows = load_store_from_database()

st.title("🍽️ Norma's Kitchenette")
st.caption("Customer Ordering Page")
st.write("Enter your info, add items, and submit your order.")

name_key = f"customer_name_{st.session_state.form_version}"
phone_key = f"customer_phone_{st.session_state.form_version}"

customer_name = st.text_input("Customer Name", key=name_key, placeholder="Enter your name")
phone_input = st.text_input("Phone Number", key=phone_key, placeholder="1234567890")
...
with st.expander("View Menu", expanded=False):
    for _, name, category, price, stock in menu_rows:
        ...
        st.write(f"{name} ({category}) - ${price:.2f} | {stock_text}")

food_names = [food.name for food in store.get_menu()]

if food_names:
    selected_food_name = st.selectbox("Select Food", food_names)

    selected_menu_row = menu_repo.get_menu_item_by_name(selected_food_name)
    available_stock = selected_menu_row[4] if selected_menu_row else 0
    ...
    if st.button("Add to Cart", use_container_width=True):
        selected_food = store.find_food(selected_food_name)
        if selected_food:
    ...
    st.warning("No menu items available right now.")
...
if st.button("Submit Order", use_container_width=True):
    ...
    if customer_name.strip() and len(phone_digits) == 10 and st.session_state.cart.items:
        ...
        for cart_item in st.session_state.cart.items:
            menu_row = menu_repo.get_menu_item_by_name(cart_item.food_item.name)
            if not menu_row or cart_item.quantity > menu_row[4]:
                stock_ok = False
                break

        if not stock_ok:
            st.warning("One or more items no longer have enough stock. Please review your order.")
        else:
